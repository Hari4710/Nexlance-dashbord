...
export default function Home() {
  // ===== OTP LOGIN - ADDED, NOTHING REMOVED =====
  ...
  // ===== YOUR ORIGINAL STATES - UNTOUCHED =====
  ...
  useEffect(() => {
    ...
    const saved = localStorage.getItem("nexlance_auth");
  ...
  useEffect(() => {
    if (otpExpiry > 0) {
      const timer = setInterval(() => setOtpExpiry((p) => (p > 0? p - 1 : 0)), 1000);
  ...
  // ===== OTP LOGIC - ADDED =====
  const sendOtp = () => {
    if (!loginEmail.includes("@")) { alert("Please enter valid email"); return; }
    const otp = Math.floor(100000 + Math.random() * 900000).toString();
    setGeneratedOtp(otp);
    setOtpExpiry(300);
    setLoginStep("otp");
    alert(`OTP sent to ${loginEmail}\n\nOTP: ${otp}\n\nIn production, this will be sent via email. Check console for OTP.`);
    console.log(`OTP for ${loginEmail}: ${otp}`);
  };

  const verifyOtp = () => {
    if (loginOtp.length!== 6) { alert("Enter 6 digit OTP"); return; }
    if (otpExpiry === 0) { alert("OTP expired. Please resend."); setLoginStep("email"); return; }
    if (loginOtp === generatedOtp) {
      localStorage.setItem("nexlance_auth", loginEmail);
      document.cookie = `finalAuthToken=${loginEmail}; path=/; max-age=604800`;
      setIsLoggedIn(true);
    } else {
      alert(`Invalid OTP. Correct OTP is ${generatedOtp} (shown for testing)`);
    }
  };
  ...
  if (!isLoggedIn) {
    return (
      <div className="min-h-screen bg-[#eef1f6] flex items-center justify-center p-4 font-sans">
        <style>{`@keyframes softGlow{0%,100%{box-shadow:0 0 8px rgba(253,230,138,0.3)}50%{box-shadow:0 0 14px rgba(196,181,253,0.35)}}.animated-logo{animation:softGlow 3s ease-in-out infinite}`}</style>
        <div className="bg-white rounded-[24px] shadow-xl border w-full max-w-[400px] overflow-hidden">
          <div className="h-[6px]" style={{ background: lightGradient }}></div>
          <div className="p-8">
            <div className="flex items-center gap-3 mb-6">
              <div className="relative w-[44px] h-[44px] bg-black rounded-xl flex items-center justify-center overflow-hidden ring-1 ring-white/20 animated-logo">
              ...
              <div><p className="font-bold text-[13px] leading-4">Nexlance Collections<br />System</p><p className="text-[10px] text-slate-400">OTP Login Required</p></div>
            </div>

            {loginStep === "email"? (
              <>
                <h2 className="font-bold text-[18px] mb-1">Login to Dashboard</h2>
                <p className="text-[12px] text-slate-500 mb-5">OTP will be sent to your email. Enter OTP to access dashboard.</p>
                <label className="text-[11px] font-medium">Email Address</label>
                <input value={loginEmail} onChange={(e) => setLoginEmail(e.target.value)} placeholder="karthick@nexlance.in" className="w-full border rounded-full px-4 py-3 text-[13px] mt-2 mb-4 focus:outline-none focus:ring-2 focus:ring-violet-200" />
                <button onClick={sendOtp} className="w-full py-3 rounded-full font-bold text-[#1e1b4b] shadow-md" style={{ background: lightGradient }}>Send OTP to Email →</button>
                <p className="text-[10px] text-slate-400 mt-4 text-center">For testing, OTP is shown in alert and console.<br />Connect Resend API for real email delivery.</p>
              </>
            ) : (
              <>
                <h2 className="font-bold text-[18px] mb-1">Enter OTP</h2>
                <p className="text-[12px] text-slate-500 mb-1">OTP sent to <b>{loginEmail}</b></p>
                <p className="text-[10px] text-emerald-600 mb-4 font-bold">Expires in {Math.floor(otpExpiry/60)}:{(otpExpiry%60).toString().padStart(2,'0')} | Demo OTP: {generatedOtp}</p>
                <label className="text-[11px] font-medium">6-Digit OTP</label>
                <input value={loginOtp} onChange={(e) => setLoginOtp(e.target.value.replace(/\D/g,'').slice(0,6))} placeholder="123456" className="w-full border rounded-full px-4 py-3 text-[22px] tracking-[10px] text-center font-bold mt-2 mb-4 focus:outline-none focus:ring-2 focus:ring-violet-200" />
                <button onClick={verifyOtp} className="w-full py-3 rounded-full font-bold text-[#1e1b4b] shadow-md" style={{ background: lightGradient }}>Verify OTP & Open Dashboard →</button>
                <div className="flex gap-2 mt-3"><button onClick={sendOtp} className="flex-1 py-2 rounded-full text-[11px] bg-slate-100">Resend OTP</button><button onClick={() => setLoginStep("email")} className="flex-1 py-2 rounded-full text-[11px] bg-slate-100">Change Email</button></div>
              </>
            )}
          </div>
        </div>
  ...
  // ===== YOUR ORIGINAL DASHBOARD - 100% UNTOUCHED =====
  return (
    <div className="min-h-screen bg-[#eef1f6] flex font-sans">
      <style>{`
        @keyframes logoGlow {
          0%, 100% { box-shadow: 0 0 10px rgba(212,176,0,0.6), 0 0 20px rgba(59,130,246,0.4), 0 0 30px rgba(139,92,246,0.3); }
          50% { box-shadow: 0 0 20px rgba(255,140,66,0.8), 0 0 30px rgba(139,92,246,0.6), 0 0 40px rgba(59,130,246,0.4); }
        }
        @keyframes logoFloat {
      ...
      <aside className="w-[270px] bg-[#0f172a] text-slate-400 flex flex-col shrink-0">
        <div className="p-4 border-b border-white/10 flex gap-3">
          ...
          <div>
            ...
            <p className="text-[8px] text-emerald-300 mt-1">✓ {loginEmail}</p>
          </div>
        </div>
          ...
          {menu.map((m) => {
            ...
            return (
              ...
              >
                ...
                {m.includes("Agent") && <span className={`text-[7px] px-1.5 py-0.5 rounded-full ${sel? "bg-black/20" : "bg-white/10"}`}>Priority Sorted</span>}
                {sel && <span>→</span>}
              </div>
            );
          })}
        ...
        <div className="p-3 border-t border-white/10">
          <button onClick={doLogout} className="w-full bg-gradient-to-r from-[#D4B000] via-[#3B82F6] to-[#FF8C42] text-[#1a1033] py-3 rounded-full text-[12px] font-bold shadow-lg">Logout →</button>
        </div>
      </aside>
      <main className="flex-1 min-w-0">
        <div className="h-[72px] bg-[#0f172a] flex items-center gap-2 px-3 relative z-[300] border-b border-white/10">
          ...
          <div className="ml-auto flex items-center gap-2">
            <div className="bg-[#1e293b] border border-white/10 px-2 py-1.5 rounded-full flex items-center gap-2"><div className="w-7 h-7 bg-gradient-to-r from-[#D4B000] to-[#FF8C42] rounded-full flex items-center justify-center text-[#1a1033] text-[10px] font-bold">K</div><div><p className="text-white text-[10px] font-bold leading-3">Karthick<br />Founder</p><p className="text-[7px] text-slate-400">{loginEmail}</p></div><span className="bg-[#a855f7]/20 text-[#d8b4fe] text-[7px] px-1.5 py-0.5 rounded-full">Founder</span></div>
        ...
        <div className="p-4 bg-[#eef1f6] min-h-[calc(100vh-72px)]">
          ...
          {active === "Audit Log & Compliance" && (
            <div className="space-y-3">
              ...
              <div className="bg-white border rounded-full p-3 flex gap-2"><input placeholder="Search by Log ID, User, Entity..." className="border rounded-full px-3 py-2 text-[11px] flex-1" /><select className="border rounded-full px-3 py-2 text-[11px]"><option>All Action Types</option></select><button className="bg-orange-500 text-white px-4 py-2 rounded-full text-[9px] font-bold">Export Immutable Log CSV</button></div>
