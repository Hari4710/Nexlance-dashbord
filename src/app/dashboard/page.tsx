...
export default function Home() {
  const [active, setActive] = useState("Dashboards & MIS");
    ...
    <div className="min-h-screen bg-[#eef1f6] flex font-sans">
      <aside className="w-[270px] bg-[#0f172a] text-slate-400 flex flex-col shrink-0">
        <div className="p-4 border-b border-white/10 flex gap-3">
          <div className="w-11 h-11 bg-black rounded-xl flex items-center justify-center p-0 overflow-hidden ring-1 ring-white/20 shadow-[0_0_15px_rgba(99,102,241,0.5)]"><img src="/logo.png" alt="Nexlance" className="h-full w-full object-contain" /></div>
          ...
          {menu.map((m) => {
            ...
            return (
              <div
                key={m}
                onClick={() => setActive(m)}
                className={`px-5 py-3.5 rounded-full cursor-pointer text-[13px] flex items-center gap-3 transition-all duration-300
                ${sel
                 ? "bg-gradient-to-r from-[#D4B000] via-[#3B82F6] via-[#8B5CF6] to-[#FF8C42] text-[#1a1033] font-bold shadow-[0_4px_20px_rgba(251,146,60,0.5)] scale-[1.02]"
                  : "hover:bg-white/10 text-slate-400 hover:text-white bg-white/[0.03] border border-white/5"}`}
              >
                ...
                {m.includes("Agent") && <span className={`text-[7px] px-2 py-0.5 rounded-full ${sel? "bg-black/20 text-[#1a1033]" : "bg-white/10"}`}>Priority Sorted</span>}
                {sel && <span className="text-[18px] font-bold">→</span>}
              </div>
            );
          })}
        </nav>
        <div className="p-3 border-t border-white/10">
          <button onClick={doLogout} className="w-full bg-gradient-to-r from-[#D4B000] via-[#3B82F6] to-[#FF8C42] text-[#1a1033] py-3 rounded-full text-[12px] font-bold shadow-lg flex items-center justify-center gap-2">Logout <span>→</span></button>
        </div>
      </aside>
      <main className="flex-1 min-w-0">
        <div className="h-[72px] bg-[#0f172a] flex items-center gap-2 px-3 relative z-[300] border-b border-white/10">
          ...
          <div className="ml-auto flex items-center gap-2">
            <div className="bg-[#1e293b] border border-white/10 px-2 py-1.5 rounded-full flex items-center gap-2"><div className="w-7 h-7 bg-gradient-to-r from-[#D4B000] to-[#FF8C42] rounded-full flex items-center justify-center text-[#1a1033] text-[10px] font-bold">K</div><div><p className="text-white text-[10px] font-bold leading-3">Karthick<br />Founder</p><p className="text-[7px] text-slate-400">karthick@nexlance.in</p></div><span className="bg-[#a855f7]/20 text-[#d8b4fe] text-[7px] px-1.5 py-0.5 rounded-full">Founder</span></div>
            <button onClick={doLogout} className="bg-gradient-to-r from-[#D4B000] via-[#3B82F6] to-[#FF8C42] text-[#1a1033] px-5 py-2 rounded-full text-[10px] font-bold shadow-lg">Logout →</button>
          </div>
        </div>
        <div className="p-4 bg-[#eef1f6] min-h-[calc(100vh-72px)]">
          {active === "Agent Worklist Queue" && (
            <div className="space-y-3">
              <div className="bg-[#0f172a] rounded-2xl p-4 flex justify-between items-center"><div className="flex gap-3 items-center"><div className="w-8 h-8 bg-gradient-to-r from-[#D4B000] to-[#3B82F6] rounded-full flex items-center justify-center">📞</div><div><h2 className="text-white font-bold text-[13px]">Agent Worklist - Priority Queue Mode</h2><p className="text-[8px] text-slate-400">Priority Order: PTP Due Today - Callback Due Today - Broken PTP</p></div></div><div className="flex items-center gap-2"><button onClick={() => setQIdx(Math.max(0, qIdx - 1))} className="w-7 h-7 bg-white/10 rounded-full flex items-center justify-center text-white">{"<"}</button><div className="bg-white/10 text-white text-[9px] px-3 py-1.5 rounded-full text-center border border-white/10">Account<br />{qIdx + 1} of {queueAccounts.length}</div><button onClick={() => setQIdx(Math.min(queueAccounts.length - 1, qIdx + 1))} className="w-7 h-7 bg-white/10 rounded-full flex items-center justify-center text-white">{">"}</button></div></div>
          ...
          {active === "Audit Log & Compliance" && (
            <div className="space-y-3">
              ...
              <div className="bg-white border rounded-full p-3 flex gap-2"><input placeholder="Search by Log ID, User, Entity..." className="border rounded-full px-3 py-2 text-[11px] flex-1" /><select className="border rounded-full px-3 py-2 text-[11px]"><option>All Action Types</option></select><button className="bg-gradient-to-r from-[#D4B000] via-[#3B82F6] to-[#FF8C42] text-[#1a1033] px-5 py-2 rounded-full text-[9px] font-bold shadow-lg">Export Immutable Log CSV →</button></div>
