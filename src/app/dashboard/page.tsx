    ...
    <div className="min-h-screen bg-[#eef1f6] flex font-sans">
      <aside className="w-[270px] bg-[#0f172a] text-slate-400 flex flex-col shrink-0">
        ...
        <nav className="p-3 space-y-1.5 flex-1">
          {menu.map((m) => {
            const sel = active === m;
            return (<div key={m} onClick={() => setActive(m)} className={`px-4 py-3 rounded-xl cursor-pointer text-[13px] flex items-center gap-3 transition-all ${sel? "bg-[#3b82f6] text-white font-bold shadow-lg" : "hover:bg-white/10"}`}><span className="text-[14px]">{m === "Dashboards & MIS"? "📊" : m.includes("Agent")? "📞" : m.includes("Allocation")? "🔀" : m.includes("Payment")? "💳" : m.includes("Client")? "🏢" : m.includes("User")? "👥" : "🛡️"}</span><span className="flex-1">{m}</span>{m.includes("Agent") && <span className="text-[7px] bg-white/10 px-1.5 py-0.5 rounded">Priority Sorted</span>}</div>);
          })}
        </nav>
      ...
      <main className="flex-1 min-w-0">
        <div className="h-[72px] bg-[#0f172a] flex items-center gap-2 px-3 relative z-[300] border-b border-white/10">
          ...
          <div className="bg-[#1e293b] border border-white/10 px-2 py-2 rounded-xl flex items-center gap-1"><span className="text-[7px] text-slate-400 leading-3">Switch<br />Active<br />Role:</span><div className="relative"><div onClick={() => { setShowRoles(!showRoles); setShow2FA(false); setShowClients(false); }} className="bg-[#0f172a] border border-white/10 text-white text-[10px] px-2 py-1.5 rounded-lg cursor-pointer flex items-center gap-2 w-[220px] justify-between"><span className="truncate">{role}</span><span>▼</span></div>{showRoles && (<div className="absolute top-[36px] left-0 w-[360px] bg-[#1a1d29] rounded-[16px] shadow-2xl z-[500] border border-white/10 overflow-hidden max-h-[70vh] overflow-y-auto">{rolesList.map((r, i) => { const sel = role === r; return (<div key={i} onClick={() => { setRole(r); setShowRoles(false); }} className={`px-4 py-3 flex justify-between items-center border-b border-white/10 hover:bg-white/10 cursor-pointer ${sel? "bg-blue-500/10" : ""}`}><span className="text-white text-[12px] font-bold">{r}</span><div className={`w-5 h-5 rounded-full border flex items-center justify-center ${sel? "border-blue-400" : "border-white/30"}`}>{sel && <div className="w-2.5 h-2.5 bg-blue-400 rounded-full"></div>}</div></div>); })}</div>)}</div></div>
          <div className="ml-auto flex items-center gap-2">
            <div className="bg-[#1e293b] border border-white/10 px-2 py-1.5 rounded-xl flex items-center gap-2"><div className="w-7 h-7 bg-blue-500 rounded-full flex items-center justify-center text-white text-[10px] font-bold">K</div><div><p className="text-white text-[10px] font-bold leading-3">Karthick<br />Founder</p><p className="text-[7px] text-slate-400">karthick@nexlance.in</p></div><span className="bg-[#a855f7]/20 text-[#d8b4fe] text-[7px] px-1.5 py-0.5 rounded">Founder</span></div>
        ...
        <div className="p-4 bg-[#eef1f6] min-h-[calc(100vh-72px)]">
          {active === "Agent Worklist Queue" && (
            <div className="space-y-3">
              <div className="bg-[#0f172a] rounded-xl p-4 flex justify-between items-center"><div className="flex gap-3 items-center"><div className="w-8 h-8 bg-emerald-500/20 rounded-lg flex items-center justify-center">📞</div><div><h2 className="text-white font-bold text-[13px]">Agent Worklist - Priority Queue Mode</h2><p className="text-[8px] text-slate-400">Priority Order: PTP Due Today - Callback Due Today - Broken PTP</p></div></div><div className="flex items-center gap-2"><button onClick={() => setQIdx(Math.max(0, qIdx - 1))} className="w-7 h-7 bg-white/10 rounded-lg flex items-center justify-center text-white">{"<"}</button><div className="bg-white/10 text-white text-[9px] px-3 py-1.5 rounded-lg text-center border border-white/10">Account<br />{qIdx + 1} of {queueAccounts.length}</div><button onClick={() => setQIdx(Math.min(queueAccounts.length - 1, qIdx + 1))} className="w-7 h-7 bg-white/10 rounded-lg flex items-center justify-center text-white">{">"}</button></div></div>
              <div className="bg-white border rounded-xl p-4"><div className="flex justify-between items-center"><div className="flex items-center gap-2"><h3 className="font-bold text-[14px]">{queueAccounts[qIdx].name}</h3><span className="bg-red-50 text-red-600 border border-red-200 text-[9px] px-2 py-0.5 rounded">{queueAccounts[qIdx].tag}</span></div><span className="bg-blue-50 text-blue-600 border border-blue-100 text-[9px] px-2.5 py-1 rounded-full font-bold">Loan ID: {queueAccounts[qIdx].loan}</span></div><div className="grid grid-cols-4 gap-4 bg-slate-50 border rounded-xl p-3 mt-3"><div><p className="text-[8px] text-slate-500 tracking-widest">MASKED PHONE</p><p className="font-bold text-[12px] mt-1">{queueAccounts[qIdx].phone}</p></div><div><p className="text-[8px] text-slate-500 tracking-widest">BORROWER CITY</p><p className="font-bold text-[12px] mt-1">{queueAccounts[qIdx].city}</p></div><div><p className="text-[8px] text-slate-500 tracking-widest">DPD BUCKET</p><p className="font-bold text-[12px] mt-1 text-red-600">{queueAccounts[qIdx].dpd}</p></div><div><p className="text-[8px] text-slate-500 tracking-widest">TOTAL DUE</p><p className="font-bold text-[12px] mt-1 text-emerald-600">{queueAccounts[qIdx].due}</p></div></div></div>
              <div className="bg-white border rounded-xl p-4"><h3 className="font-bold text-[12px]">Capture Borrower Disposition</h3><div className="grid grid-cols-2 gap-3 mt-3"><div><label className="text-[9px] text-slate-500">Contact Mode</label><select value={contactMode} onChange={(e) => setContactMode(e.target.value)} className="w-full border rounded-lg px-3 py-2 text-[11px] mt-1 bg-white"><option>Call</option><option>Field Visit</option><option>WhatsApp</option><option>Email</option></select></div><div><label className="text-[9px] text-slate-500">Disposition Code</label><select value={disposition} onChange={(e) => setDisposition(e.target.value)} className="w-full border rounded-lg px-3 py-2 text-[11px] mt-1 bg-white"><option>PTP Taken</option><option>Callback</option><option>Broken PTP</option><option>RTP</option><option>Customer Not Reachable</option></select></div></div><div className="grid grid-cols-2 gap-3 mt-3 bg-emerald-50/70 border border-emerald-100 rounded-xl p-3"><div><label className="text-[9px] text-emerald-700 font-bold">Promised Amount</label><input value={promisedAmt} onChange={(e) => setPromisedAmt(e.target.value)} placeholder="e.g. 32000" className="w-full border rounded-lg px-3 py-2 text-[11px] mt-1" /></div><div><label className="text-[9px] text-slate-600">Promised Payment Date</label><select value={promisedDate} onChange={(e) => setPromisedDate(e.target.value)} className="w-full border rounded-lg px-3 py-2 text-[11px] mt-1 bg-white"><option value="">Select Date</option><option>09/18/2026</option><option>09/19/2026</option></select></div></div><div className="mt-3"><label className="text-[9px] text-slate-500">Next Follow-up Action Date</label><select value={followUpDate} onChange={(e) => setFollowUpDate(e.target.value)} className="w-full border rounded-lg px-3 py-2 text-[11px] mt-1 bg-white"><option>09/17/2026</option><option>09/18/2026</option><option>09/19/2026</option></select></div><div className="mt-3"><label className="text-[9px] text-slate-500">Activity Remarks</label><textarea value={remarks} onChange={(e) => setRemarks(e.target.value)} placeholder="Enter detailed call summary, borrower response, or commitment details..." className="w-full border rounded-lg px-3 py-2 text-[11px] mt-1 h-[60px] resize-none"></textarea></div><button onClick={saveDisposition} className="w-full bg-[#3b82f6] hover:bg-blue-700 text-white py-2.5 rounded-xl mt-4 font-bold text-[12px]">Save Disposition & Advance Queue</button></div>
              <div className="bg-white border rounded-xl p-4">
                <h3 className="font-bold text-[11px]">Prior Activity Log History ({activityLogs.length})</h3>
                <div className="space-y-2 mt-3">{activityLogs.map((log, i) => (<div key={i} className="bg-blue-50/50 border border-blue-100 rounded-lg p-3 flex justify-between"><div><p className="font-bold text-[11px] text-blue-700">{log.type}</p><p className="text-[10px] text-slate-700 mt-0.5">Promised Rs. {log.amt} partial payment on {log.date}</p><p className="text-[8px] text-slate-400 mt-1">Agent: {log.agent}</p></div><div className="text-right"><p className="text-[9px] text-slate-500">{log.time}</p><p className="text-[8px] text-slate-400 mt-6">Next: {log.next}</p></div></div>))}</div>
              </div>
            </div>
          )}
          ...
          {active === "Allocation & Assignment" && (
            <div className="space-y-3">
              ...
              <div className="bg-white border rounded-xl p-4"><div className="flex justify-between items-center mb-3"><h3 className="font-bold text-[12px]">Upload Client Delinquent Allocation File (CSV)</h3><button className="text-[9px] bg-blue-50 text-blue-600 px-3 py-1.5 rounded-lg">Download Sample CSV Template</button></div><div className="border-2 border-dashed border-slate-200 rounded-xl p-8 text-center bg-slate-50/50"><p className="font-bold text-[12px]">Choose CSV File to Upload & Validate</p><button onClick={() => alert(`CSV Uploaded`)} className="mt-3 bg-[#3b82f6] text-white px-4 py-1.5 rounded-lg text-[11px] font-bold">Choose File</button></div></div>
          ...
          {active === "Payment Recon & Queue" && (
            <div className="space-y-3">
              ...
              <div className="bg-white border rounded-xl p-4"><h3 className="font-bold text-[12px] mb-3">Upload Client Daily Paid File (CSV)</h3><div className="border-2 border-dashed border-slate-200 rounded-xl p-8 text-center bg-slate-50/30"><p className="font-bold text-[12px]">Choose Daily Payment CSV</p><button onClick={() => alert(`Payment Reconciled`)} className="mt-3 bg-emerald-600 text-white px-4 py-1.5 rounded-lg text-[11px] font-bold">Choose File</button></div></div>
          ...
          {active === "Audit Log & Compliance" && (
            <div className="space-y-3">
              ...
              <div className="bg-white border rounded-xl p-3 flex gap-2"><input placeholder="Search by Log ID, User, Entity..." className="border rounded-lg px-3 py-2 text-[11px] flex-1" /><select className="border rounded-lg px-3 py-2 text-[11px]"><option>All Action Types</option></select><button className="bg-orange-500 text-white px-4 py-2 rounded-lg text-[9px] font-bold">Export Immutable Log CSV</button></div>
              <div className="bg-white border rounded-xl p-4"><h3 className="font-bold text-[11px] mb-3">System Evidentiary Log Trail (4 Records)</h3><table className="w-full text-[9px]"><thead className="bg-slate-50 border-b"><tr><th className="text-left p-2">TIMESTAMP</th><th>LOG ID</th><th>ACTOR</th><th>ACTION</th><th>TARGET ENTITY</th></tr></thead><tbody><tr className="border-b"><td className="p-2.5">17/9/2026, 10:18:42 pm</td><td className="font-mono">LOG_1789663722962_62</td><td className="font-bold">Karthick Founder</td><td><span className="bg-slate-100 border px-2 py-0.5 rounded">LOGIN</span></td><td>mis_export (CLI_KISSHT)</td></tr><tr className="border-b"><td className="p-2.5">17/9/2026, 10:18:33 pm</td><td className="font-mono">LOG_1789663713807_869</td><td className="font-bold">Karthick Founder</td><td><span className="bg-slate-100 border px-2 py-0.5 rounded">LOGIN</span></td><td>mis_export (CLI_KISSHT)</td></tr><tr className="border-b"><td className="p-2.5">10/9/2026, 10:53:23 am</td><td className="font-mono">LOG_1789017803843_415</td><td className="font-bold">Karthick Founder</td><td><span className="bg-slate-100 border px-2 py-0.5 rounded">LOGOUT</span></td><td>session (USR_FOUNDER)</td></tr><tr><td className="p-2.5">10/9/2026, 10:50:04 am</td><td className="font-mono">LOG_1789017604953_148</td><td className="font-bold">Karthick Founder</td><td><span className="bg-slate-100 border px-2 py-0.5 rounded">LOGIN</span></td><td>mis_export (CLI_KISSHT)</td></tr></tbody></table></div>
            </div>
          )}
        </div>
      </main>
