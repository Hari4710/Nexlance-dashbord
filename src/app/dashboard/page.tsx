...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-[#eef1f6] flex font-sans">
      <style>{`
        @keyframes logoGlow {
          0%, 100% { box-shadow: 0 0 12px #FFD700, 0 0 22px #3B82F6, 0 0 32px #A855F7; }
          25% { box-shadow: 0 0 15px #00D2FF, 0 0 25px #6366F1, 0 0 35px #EC4899; }
          50% { box-shadow: 0 0 18px #A855F7, 0 0 28px #EC4899, 0 0 38px #FF8C42; }
          75% { box-shadow: 0 0 15px #FF8C42, 0 0 25px #FFD700, 0 0 35px #3B82F6; }
        }
        @keyframes floatAnim { 0%, 100% { transform: translateY(0) scale(1); } 50% { transform: translateY(-3px) scale(1.05); } }
       .animated-logo { animation: logoGlow 3s ease-in-out infinite, floatAnim 3s ease-in-out infinite; }
       .logo-gradient-bg { background: linear-gradient(90deg, #FFD700, #7ED957, #00D2FF, #3B82F6, #6366F1, #A855F7, #EC4899, #FF8C69, #FF8C42); }
      `}</style>

      <aside className="w-[270px] bg-[#0f172a] text-slate-400 flex flex-col shrink-0">
        <div className="p-4 border-b border-white/10 flex gap-3">
          {/* ANIMATED LOGO WITH ALL LOGO COLOURS */}
          <div className="relative w-12 h-12 bg-black rounded-xl flex items-center justify-center p-1 overflow-hidden ring-1 ring-white/20 animated-logo">
            <div className="absolute inset-0 logo-gradient-bg opacity-30 animate-pulse rounded-xl"></div>
        ...
        <nav className="p-3 space-y-2.5 flex-1">
          {menu.map((m) => {
            const sel = active === m;
            return (
              <div
                key={m}
                onClick={() => setActive(m)}
                className={`px-4 py-3.5 rounded-full cursor-pointer text-[13px] flex items-center gap-3 transition-all duration-500
                ${sel
               ? "bg-gradient-to-r from-[#FFD700] via-[#7ED957] via-[#00D2FF] via-[#3B82F6] via-[#6366F1] via-[#A855F7] via-[#EC4899] to-[#FF8C42] text-[#1a1033] font-bold shadow-[0_4px_25px_rgba(236,72,153,0.5)] scale-[1.03]"
                  : "hover:bg-white/10 text-slate-400 hover:text-white bg-white/[0.04] border border-white/5"}`}
              >
                ...
                {m.includes("Agent") && <span className={`text-[7px] px-2 py-0.5 rounded-full font-bold ${sel? "bg-black/20 text-[#1a1033]" : "bg-white/10"}`}>Priority Sorted</span>}
                {sel && <span className="text-[16px]">→</span>}
              </div>
            );
          })}
        </nav>
        <div className="p-3 border-t border-white/10">
          <button onClick={doLogout} className="w-full bg-gradient-to-r from-[#FFD700] via-[#00D2FF] via-[#A855F7] via-[#EC4899] to-[#FF8C42] text-[#1a1033] py-3 rounded-full text-[12px] font-bold shadow-[0_4px_20px_rgba(236,72,153,0.4)] flex items-center justify-center gap-2">Logout <span>→</span></button>
        </div>
      </aside>
      <main className="flex-1 min-w-0">
        <div className="h-[72px] bg-[#0f172a] flex items-center gap-2 px-3 relative z-[300] border-b border-white/10">
          ...
          <div className="ml-auto flex items-center gap-2">
            <div className="bg-[#1e293b] border border-white/10 px-2 py-1.5 rounded-full flex items-center gap-2"><div className="w-7 h-7 bg-gradient-to-r from-[#FFD700] via-[#A855F7] to-[#FF8C42] rounded-full flex items-center justify-center text-[#1a1033] text-[10px] font-bold">K</div><div><p className="text-white text-[10px] font-bold leading-3">Karthick<br />Founder</p><p className="text-[7px] text-slate-400">karthick@nexlance.in</p></div><span className="bg-[#a855f7]/20 text-[#d8b4fe] text-[7px] px-1.5 py-0.5 rounded-full">Founder</span></div>
            <button onClick={doLogout} className="bg-gradient-to-r from-[#FFD700] via-[#A855F7] to-[#FF8C42] text-[#1a1033] px-5 py-2.5 rounded-full text-[10px] font-bold shadow-lg">Logout →</button>
          </div>
        </div>
        <div className="p-4 bg-[#eef1f6] min-h-[calc(100vh-72px)]">
          {active === "Agent Worklist Queue" && (
            <div className="space-y-3">
              <div className="bg-[#0f172a] rounded-2xl p-4 flex justify-between items-center border border-white/5"><div className="flex gap-3 items-center"><div className="w-8 h-8 bg-gradient-to-r from-[#FFD700] to-[#00D2FF] rounded-full flex items-center justify-center">📞</div><div><h2 className="text-white font-bold text-[13px]">Agent Worklist - Priority Queue Mode</h2><p className="text-[8px] text-slate-400">Priority Order: PTP Due Today - Callback Due Today - Broken PTP</p></div></div><div className="flex items-center gap-2"><button onClick={() => setQIdx(Math.max(0, qIdx - 1))} className="w-7 h-7 bg-white/10 rounded-full flex items-center justify-center text-white">{"<"}</button><div className="bg-white/10 text-white text-[9px] px-3 py-1.5 rounded-full text-center border border-white/10">Account<br />{qIdx + 1} of {queueAccounts.length}</div><button onClick={() => setQIdx(Math.min(queueAccounts.length - 1, qIdx + 1))} className="w-7 h-7 bg-white/10 rounded-full flex items-center justify-center text-white">{">"}</button></div></div>
              <div className="bg-white border rounded-2xl p-4 shadow-sm"><div className="flex justify-between items-center"><div className="flex items-center gap-2"><h3 className="font-bold text-[14px]">{queueAccounts[qIdx].name}</h3><span className="bg-gradient-to-r from-[#FFD700] to-[#FF8C42] text-[#1a1033] border text-[9px] px-2 py-0.5 rounded-full font-bold">{queueAccounts[qIdx].tag}</span></div><span className="bg-blue-50 text-blue-600 border border-blue-100 text-[9px] px-2.5 py-1 rounded-full font-bold">Loan ID: {queueAccounts[qIdx].loan}</span></div><div className="grid grid-cols-4 gap-4 bg-slate-50 border rounded-xl p-3 mt-3"><div><p className="text-[8px] text-slate-500 tracking-widest">MASKED PHONE</p><p className="font-bold text-[12px] mt-1">{queueAccounts[qIdx].phone}</p></div><div><p className="text-[8px] text-slate-500 tracking-widest">BORROWER CITY</p><p className="font-bold text-[12px] mt-1">{queueAccounts[qIdx].city}</p></div><div><p className="text-[8px] text-slate-500 tracking-widest">DPD BUCKET</p><p className="font-bold text-[12px] mt-1 text-red-600">{queueAccounts[qIdx].dpd}</p></div><div><p className="text-[8px] text-slate-500 tracking-widest">TOTAL DUE</p><p className="font-bold text-[12px] mt-1 text-emerald-600">{queueAccounts[qIdx].due}</p></div></div></div>
              <div className="bg-white border rounded-2xl p-4 shadow-sm"><h3 className="font-bold text-[12px]">Capture Borrower Disposition</h3><div className="grid grid-cols-2 gap-3 mt-3"><div><label className="text-[9px] text-slate-500">Contact Mode</label><select value={contactMode} onChange={(e) => setContactMode(e.target.value)} className="w-full border rounded-full px-3 py-2 text-[11px] mt-1 bg-white"><option>Call</option><option>Field Visit</option><option>WhatsApp</option><option>Email</option></select></div><div><label className="text-[9px] text-slate-500">Disposition Code</label><select value={disposition} onChange={(e) => setDisposition(e.target.value)} className="w-full border rounded-full px-3 py-2 text-[11px] mt-1 bg-white"><option>PTP Taken</option><option>Callback</option><option>Broken PTP</option><option>RTP</option><option>Customer Not Reachable</option></select></div></div><div className="grid grid-cols-2 gap-3 mt-3 bg-gradient-to-r from-[#FFD700]/10 via-[#00D2FF]/10 to-[#EC4899]/10 border border-purple-100 rounded-2xl p-3"><div><label className="text-[9px] text-emerald-700 font-bold">Promised Amount</label><input value={promisedAmt} onChange={(e) => setPromisedAmt(e.target.value)} placeholder="e.g. 32000" className="w-full border rounded-full px-3 py-2 text-[11px] mt-1" /></div><div><label className="text-[9px] text-slate-600">Promised Payment Date</label><select value={promisedDate} onChange={(e) => setPromisedDate(e.target.value)} className="w-full border rounded-full px-3 py-2 text-[11px] mt-1 bg-white"><option value="">Select Date</option><option>09/18/2026</option><option>09/19/2026</option></select></div></div><div className="mt-3"><label className="text-[9px] text-slate-500">Next Follow-up Action Date</label><select value={followUpDate} onChange={(e) => setFollowUpDate(e.target.value)} className="w-full border rounded-full px-3 py-2 text-[11px] mt-1 bg-white"><option>09/17/2026</option><option>09/18/2026</option><option>09/19/2026</option></select></div><div className="mt-3"><label className="text-[9px] text-slate-500">Activity Remarks</label><textarea value={remarks} onChange={(e) => setRemarks(e.target.value)} placeholder="Enter detailed call summary, borrower response, or commitment details..." className="w-full border rounded-2xl px-3 py-2 text-[11px] mt-1 h-[60px] resize-none"></textarea></div><button onClick={saveDisposition} className="w-full bg-gradient-to-r from-[#FFD700] via-[#00D2FF] via-[#A855F7] via-[#EC4899] to-[#FF8C42] text-[#1a1033] py-3 rounded-full mt-4 font-bold text-[12px] shadow-[0_4px_20px_rgba(236,72,153,0.4)]">Save Disposition & Advance Queue →</button></div>
              <div className="bg-white border rounded-2xl p-4 shadow-sm">
                <h3 className="font-bold text-[11px]">Prior Activity Log History ({activityLogs.length})</h3>
                <div className="space-y-2 mt-3">{activityLogs.map((log, i) => (<div key={i} className="bg-blue-50/50 border border-blue-100 rounded-2xl p-3 flex justify-between"><div><p className="font-bold text-[11px] text-blue-700">{log.type}</p><p className="text-[10px] text-slate-700 mt-0.5">Promised Rs. {log.amt} partial payment on {log.date}</p><p className="text-[8px] text-slate-400 mt-1">Agent: {log.agent}</p></div><div className="text-right"><p className="text-[9px] text-slate-500">{log.time}</p><p className="text-[8px] text-slate-400 mt-6">Next: {log.next}</p></div></div>))}</div>
              </div>
            </div>
          )}
          {active === "Dashboards & MIS" && (
            <div className="space-y-4">
              <div className="bg-[#0f172a] rounded-2xl p-4 flex justify-between items-center"><div><h2 className="text-white font-bold text-[13px]">Executive Oversight Dashboard - {client}</h2><p className="text-[9px] text-slate-400">Role: {role} | Users: {users.length} | Live Graph</p></div><div className="flex gap-2"><div className="relative"><div onClick={() => setShowClients(!showClients)} className="bg-transparent border border-blue-500 text-white text-[10px] px-3 py-2 rounded-full cursor-pointer w-[180px] flex justify-between">{client} <span>▼</span></div>{showClients && <div className="absolute top-[36px] right-0 w-[300px] bg-[#1a1d29] rounded-xl shadow-2xl z-[100] border border-white/10 overflow-hidden">{clients.map((c, i) => <div key={i} onClick={() => { setClient(c); setShowClients(false); }} className="px-4 py-3 text-white text-[12px] hover:bg-white/10 cursor-pointer border-b border-white/5">{c}</div>)}</div>}</div><button onClick={() => alert(`MIS Export`)} className="bg-gradient-to-r from-[#FFD700] via-[#A855F7] to-[#FF8C42] text-[#1a1033] text-[9px] px-4 py-2 rounded-full font-bold shadow-lg">1-Click MIS Export →</button></div></div>
              <div className="bg-white border rounded-2xl p-4 shadow-sm"><h3 className="font-bold text-[12px] mb-2">DPD Bucket Resolution Summary - {client}</h3><table className="w-full text-[10px]"><thead className="bg-slate-50 border-b"><tr><th className="text-left p-2">DPD Bucket</th><th>Allocated</th><th>Total POS</th><th>Resolved</th><th className="text-right">%</th></tr></thead><tbody><tr className="border-b"><td className="p-2.5 font-bold">1-30 DPD</td><td className="text-center">3</td><td className="text-center">₹4,65,500</td><td className="text-center">0</td><td className="text-right text-blue-600 font-bold">0%</td></tr><tr className="border-b"><td className="p-2.5 font-bold">31-60 DPD</td><td className="text-center">1</td><td className="text-center">₹97,500</td><td className="text-center">0</td><td className="text-right text-blue-600">0%</td></tr><tr className="border-b"><td className="p-2.5 font-bold">61-90 DPD</td><td className="text-center">3</td><td className="text-center">₹4,68,000</td><td className="text-center">1</td><td className="text-right text-blue-600">33%</td></tr><tr><td className="p-2.5 font-bold">90+ DPD</td><td className="text-center">1</td><td className="text-center">₹2,42,000</td><td className="text-center">0</td><td className="text-right text-blue-600">0%</td></tr></tbody></table></div>
              <div className="bg-white border rounded-2xl p-4 shadow-sm"><div className="flex justify-between mb-3"><h3 className="font-bold text-[12px]">Daily Collections Trend (INR) - Running Live</h3><span className="text-[9px] bg-gradient-to-r from-[#FFD700] to-[#FF8C42] text-[#1a1033] px-3 py-1 rounded-full font-bold">Tick: {tick}</span></div><div className="relative h-[220px] w-full"><svg viewBox="0 0 700 200" className="w-full h-full"><defs><linearGradient id="gg1" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#A855F7" stopOpacity="0.35" /><stop offset="100%" stopColor="#00D2FF" stopOpacity="0" /></linearGradient><linearGradient id="lineGrad" x1="0" y1="0" x2="1" y2="0"><stop offset="0%" stopColor="#FFD700" /><stop offset="25%" stopColor="#00D2FF" /><stop offset="50%" stopColor="#A855F7" /><stop offset="75%" stopColor="#EC4899" /><stop offset="100%" stopColor="#FF8C42" /></linearGradient></defs><path d={areaD} fill="url(#gg1)" /><path d={pathD} fill="none" stroke="url(#lineGrad)" strokeWidth="3" />{pts.map((p, i) => <circle key={i} cx={p.x} cy={p.y} r="5" fill="white" stroke="#A855F7" strokeWidth="2" />)}</svg></div></div>
            </div>
          )}
          {active === "Allocation & Assignment" && (
            <div className="space-y-3">
              <div className="bg-[#0f172a] rounded-2xl p-4 flex justify-between items-center"><div className="flex gap-3 items-center"><div className="w-8 h-8 bg-white/10 rounded-full flex items-center justify-center">🔀</div><div><h2 className="text-white font-bold text-[13px]">Allocation Batch Upload & Assignment Engine</h2><p className="text-[8px] text-slate-400">Template validation, row-level error reporting, deduplication, Round Robin & Filter-based Assignment.</p></div></div><div className="flex gap-2"><div className="bg-gradient-to-r from-[#FFD700] via-[#00D2FF] to-[#A855F7] text-[#1a1033] text-[9px] px-3 py-2 rounded-full text-center font-bold">1<br />Upload<br />Batch</div><div className="bg-white/10 text-slate-400 text-[9px] px-3 py-2 rounded-full text-center">2<br />Round<br />Robin</div><div className="bg-white/10 text-slate-400 text-[9px] px-3 py-2 rounded-full text-center">3<br />Manual<br />Bulk<br />Filter</div></div></div>
              <div className="bg-white border rounded-2xl p-4 shadow-sm"><div className="flex justify-between items-center mb-3"><h3 className="font-bold text-[12px]">Upload Client Delinquent Allocation File (CSV)</h3><button className="text-[9px] bg-blue-50 text-blue-600 px-3 py-1.5 rounded-full">Download Sample CSV Template</button></div><div className="border-2 border-dashed border-slate-200 rounded-2xl p-8 text-center bg-gradient-to-br from-[#FFD700]/5 via-[#00D2FF]/5 to-[#EC4899]/5"><p className="font-bold text-[12px]">Choose CSV File to Upload & Validate</p><button onClick={() => alert(`CSV Uploaded`)} className="mt-3 bg-gradient-to-r from-[#FFD700] via-[#00D2FF] via-[#A855F7] via-[#EC4899] to-[#FF8C42] text-[#1a1033] px-6 py-2 rounded-full text-[11px] font-bold shadow-lg">Choose File →</button></div></div>
              <div className="bg-white border rounded-2xl p-4 shadow-sm"><h3 className="font-bold text-[11px] mb-3">Ingested Batches History (2)</h3><div className="space-y-2"><div className="border rounded-full p-3 flex justify-between"><div><p className="font-bold text-[11px]">Kissht_Delinquent_Sep2026.csv</p><p className="text-[9px] text-slate-500">Kissht Digital Finance - Records: 15</p></div><span className="bg-blue-50 text-blue-600 px-2 py-0.5 rounded-full text-[8px]">2026-09</span></div><div className="border rounded-full p-3 flex justify-between"><div><p className="font-bold text-[11px]">HDFC_Personal_Sep2026.csv</p><p className="text-[9px] text-slate-500">HDFC Bank Personal Loans - Records: 10</p></div><span className="bg-blue-50 text-blue-600 px-2 py-0.5 rounded-full text-[8px]">2026-09</span></div></div></div>
            </div>
          )}
          {active === "Payment Recon & Queue" && (
            <div className="space-y-3">
              ...
              <div className="bg-white border rounded-2xl p-4 shadow-sm"><h3 className="font-bold text-[12px] mb-3">Upload Client Daily Paid File (CSV)</h3><div className="border-2 border-dashed border-slate-200 rounded-2xl p-8 text-center bg-slate-50/30"><p className="font-bold text-[12px]">Choose Daily Payment CSV</p><button onClick={() => alert(`Payment Reconciled`)} className="mt-3 bg-gradient-to-r from-[#FFD700] via-[#A855F7] to-[#FF8C42] text-[#1a1033] px-6 py-2 rounded-full text-[11px] font-bold shadow-lg">Choose File →</button></div></div>
              <div className="bg-white border rounded-2xl p-4 shadow-sm"><h3 className="font-bold text-[11px] mb-3">Reconciled Payment Stream (1)</h3><div className="bg-emerald-50 border border-emerald-200 rounded-full p-3 flex justify-between"><div><p className="font-bold text-[11px]">Loan: HDFC-PL-77113</p><p className="text-[9px] text-slate-500">NEFT - 2026 09 04</p></div><span className="font-bold text-emerald-700 text-[12px]">₹2,01,000</span></div></div>
            </div>
          )}
          {active === "Client Master" && (
            <div className="space-y-3">
              <div className="bg-[#0f172a] rounded-2xl p-4 flex justify-between items-center"><div className="flex gap-3 items-center"><div className="w-8 h-8 bg-white/10 rounded-full flex items-center justify-center">🏢</div><div><h2 className="text-white font-bold text-[13px]">Client Master & Contract Governance</h2><p className="text-[8px] text-slate-400">Manage NBFC clients, contract durations, commission rates, and automated retention purge windows.</p></div></div><button className="bg-gradient-to-r from-[#FFD700] via-[#00D2FF] via-[#A855F7] to-[#FF8C42] text-[#1a1033] text-[9px] px-4 py-2.5 rounded-full font-bold shadow-lg">+ Add New NBFC Client →</button></div>
              <div className="bg-white border rounded-2xl p-4 shadow-sm"><table className="w-full text-[9px]"><thead className="bg-slate-50 border-b"><tr><th className="text-left p-2">CLIENT ID</th><th>CLIENT NAME</th><th>CONTRACT WINDOW</th><th>COMMISSION STRUCTURE</th><th>DATA RETENTION WINDOW</th><th>STATUS</th></tr></thead><tbody><tr className="border-b"><td className="p-3 font-bold">CLI_KISSHT</td><td className="font-bold">Kissht Digital Finance</td><td>2025-01-01 to 2026-12-31</td><td className="text-blue-600 font-bold">8.5% of collection</td><td className="text-purple-600">90 Days Purge Window</td><td><span className="bg-green-100 text-green-700 px-2 py-0.5 rounded-full text-[8px]">Active</span></td></tr><tr className="border-b"><td className="p-3 font-bold">CLI_HDFC</td><td className="font-bold">HDFC Bank Personal Loans</td><td>2024-06-01 to 2027-05-31</td><td className="text-blue-600 font-bold">10% for 90+ DPD, 6% for 30 DPD</td><td className="text-purple-600">180 Days Purge Window</td><td><span className="bg-green-100 text-green-700 px-2 py-0.5 rounded-full text-[8px]">Active</span></td></tr><tr><td className="p-3 font-bold">CLI_BAJAJ</td><td className="font-bold">Bajaj Finance Consumer Durable</td><td>2025-03-15 to 2026-09-15</td><td className="font-bold">7.5% flat</td><td className="text-purple-600">120 Days Purge Window</td><td><span className="bg-green-100 text-green-700 px-2 py-0.5 rounded-full text-[8px]">Active</span></td></tr></tbody></table></div>
            </div>
          )}
          {active === "User & Role Admin" && (
            <div className="space-y-3">
              <div className="bg-[#0f172a] rounded-2xl p-4 flex justify-between items-center"><div className="flex gap-3 items-center"><div className="w-8 h-8 bg-white/10 rounded-full flex items-center justify-center">👥</div><div><h2 className="text-white font-bold text-[13px]">User & Access Management - Security Compliance</h2><p className="text-[8px] text-slate-400">Application-owned identity. Single-click disablement immediately terminates active sessions and logs to audit trail.</p></div></div><button onClick={() => setShowProvision(true)} className="bg-gradient-to-r from-[#FFD700] via-[#A855F7] to-[#FF8C42] text-[#1a1033] px-5 py-2.5 rounded-full text-[9px] font-bold shadow-lg">Provision New User →</button></div>
              <div className="bg-white border rounded-2xl overflow-hidden shadow-sm"><table className="w-full text-[9px]"><thead className="bg-slate-50 border-b"><tr><th className="text-left p-2">AGENT / USER ID</th><th>FULL NAME & EMAIL</th><th>ROLE</th><th>TEAM LEADER</th><th>2FA & AUTH STATUS</th><th>ACTION</th></tr></thead><tbody>{users.map((a) => <tr key={a.id} className="border-b"><td className="p-2.5 font-bold text-[10px]">{a.id}</td><td className="p-2.5"><p className="font-bold text-[10px]">{a.name}</p><p className="text-[8px] text-slate-500">{a.email}</p></td><td className="p-2.5"><span className="bg-purple-100 text-purple-700 px-2 py-0.5 rounded-full text-[8px] font-bold">{a.role}</span></td><td className="p-2.5 text-[9px]">{a.tl}</td><td className="p-2.5"><span className="bg-emerald-50 text-emerald-700 border border-emerald-200 px-2 py-0.5 rounded-full text-[8px]">TOTP 2FA Active</span></td><td className="p-2.5"><button onClick={() => { if (confirm(`Disable ${a.name}?`)) setUsers(users.filter((x) => x.id!== a.id)); }} className="bg-red-600 text-white px-3 py-1.5 rounded-full text-[8px] font-bold">1-Click Disable</button></td></tr>)}</tbody></table></div>
              {showProvision && (
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[700] flex items-center justify-center p-4">
                  <div className="bg-white rounded-[20px] w-full max-w-[420px] shadow-2xl overflow-hidden border">
                    <div className="p-6"><h3 className="font-bold text-[14px] mb-4">Provision New Application User</h3><div className="space-y-4"><div><label className="text-[11px] font-medium">Full Name</label><input value={form.fullName} onChange={(e) => setForm({...form, fullName: e.target.value })} placeholder="e.g. Ramesh Kumar" className="w-full border rounded-full px-3 py-2.5 text-[12px] mt-1.5" /></div><div><label className="text-[11px] font-medium">Registered Contact Email</label><input value={form.email} onChange={(e) => setForm({...form, email: e.target.value })} placeholder="ramesh@nexlance.in" className="w-full border rounded-full px-3 py-2.5 text-[12px] mt-1.5" /></div><div className="grid grid-cols-2 gap-3"><div><label className="text-[11px] font-medium">Assigned Role</label><select value={form.role} onChange={(e) => setForm({...form, role: e.target.value })} className="w-full border rounded-full px-3 py-2.5 text-[12px] mt-1.5 bg-white"><option>Agent</option><option>Team Leader</option><option>Ops Manager</option><option>Founder</option><option>Auditor</option></select></div><div><label className="text-[11px] font-medium">Assign Team Leader</label><select value={form.teamLeader} onChange={(e) => setForm({...form, teamLeader: e.target.value })} className="w-full border rounded-full px-3 py-2.5 text-[12px] mt-1.5 bg-white"><option>Select Team Leader</option>{teamLeaders.map((t) => <option key={t}>{t}</option>)}</select></div></div><div className="flex justify-end gap-2 pt-2"><button onClick={() => setShowProvision(false)} className="bg-slate-100 text-slate-600 px-4 py-2 rounded-full text-[11px]">Cancel</button><button onClick={provisionUser} className="bg-gradient-to-r from-[#FFD700] via-[#A855F7] to-[#FF8C42] text-[#1a1033] px-5 py-2 rounded-full text-[11px] font-bold">Provision User →</button></div></div></div>
                  </div>
                </div>
              )}
            </div>
          ...
          {active === "Audit Log & Compliance" && (
            <div className="space-y-3">
              ...
              <div className="bg-white border rounded-full p-3 flex gap-2 shadow-sm"><input placeholder="Search by Log ID, User, Entity..." className="border rounded-full px-3 py-2 text-[11px] flex-1" /><select className="border rounded-full px-3 py-2 text-[11px]"><option>All Action Types</option></select><button className="bg-gradient-to-r from-[#FFD700] via-[#A855F7] to-[#FF8C42] text-[#1a1033] px-5 py-2 rounded-full text-[9px] font-bold shadow-lg">Export Immutable Log CSV →</button></div>
              <div className="bg-white border rounded-2xl p-4 shadow-sm"><h3 className="font-bold text-[11px] mb-3">System Evidentiary Log Trail (4 Records)</h3><table className="w-full text-[9px]"><thead className="bg-slate-50 border-b"><tr><th className="text-left p-2">TIMESTAMP</th><th>LOG ID</th><th>ACTOR</th><th>ACTION</th><th>TARGET ENTITY</th></tr></thead><tbody><tr className="border-b"><td className="p-2.5">17/9/2026, 10:18:42 pm</td><td className="font-mono">LOG_1789663722962_62</td><td className="font-bold">Karthick Founder</td><td><span className="bg-slate-100 border px-2 py-0.5 rounded-full">LOGIN</span></td><td>mis_export (CLI_KISSHT)</td></tr><tr className="border-b"><td className="p-2.5">17/9/2026, 10:18:33 pm</td><td className="font-mono">LOG_1789663713807_869</td><td className="font-bold">Karthick Founder</td><td><span className="bg-slate-100 border px-2 py-0.5 rounded-full">LOGIN</span></td><td>mis_export (CLI_KISSHT)</td></tr><tr className="border-b"><td className="p-2.5">10/9/2026, 10:53:23 am</td><td className="font-mono">LOG_1789017803843_415</td><td className="font-bold">Karthick Founder</td><td><span className="bg-slate-100 border px-2 py-0.5 rounded-full">LOGOUT</span></td><td>session (USR_FOUNDER)</td></tr><tr><td className="p-2.5">10/9/2026, 10:50:04 am</td><td className="font-mono">LOG_1789017604953_148</td><td className="font-bold">Karthick Founder</td><td><span className="bg-slate-100 border px-2 py-0.5 rounded-full">LOGIN</span></td><td>mis_export (CLI_KISSHT)</td></tr></tbody></table></div>
            </div>
          )}
        </div>
      </main>
