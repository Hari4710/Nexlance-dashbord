...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen bg-[#0B1020] flex flex-col items-center justify-center p-4 font-sans">
      ...
      {/* ANIMATION LOGO - 2nd pic la */}
      <div className="relative mb-4">
        <img
          src="/logo.png"
          alt="Nexlance Logo"
          className="w-28 h-28 object-contain drop-shadow-[0_0_25px_rgba(56,189,248,0.7)]"
          style={{ animation: 'floatLogo 3s ease-in-out infinite' }}
        />
        <style>{`
          @keyframes floatLogo {
            0%, 100% { transform: translateY(0px) scale(1); filter: brightness(1); }
            50% { transform: translateY(-10px) scale(1.08); filter: brightness(1.2) drop-shadow(0 0 30px rgba(168,85,247,0.8)); }
          }
        `}</style>
      </div>

      <h1 className="text-white text-xl font-bold mt-1">Nexlance Collections System</h1>
      ...
      <div className="bg-white rounded-2xl p-6 w-full max-w-[380px] mt-6 shadow-2xl">
        ...
        <button onClick={doLogin} className="w-full bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl py-3 mt-5 font-bold text-sm transition-all">
